System.register(["../config/SelfClosingElements", "../config/UnnestableElements", "he", "../config/NamespaceURI"], function (exports_1, context_1) {
    ...
    return {
        ...
        execute: function () {
            MARKUP_REGEXP = /<(\/?)([a-z][-.0-9_a-z]*)\s*([^>]*?)(\/?)>/gi;
            COMMENT_REGEXP = /<!--(.*?)-->|<([!?])([^>]*)>/gi;
            DOCUMENT_TYPE_ATTRIBUTE_REGEXP = /"([^"]+)"/gm;
            ATTRIBUTE_REGEXP = /([^\s=]+)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|(\S+)))/gms;
            XMLNS_ATTRIBUTE_REGEXP = /xmlns[ ]*=[ ]*"([^"]+)"/;
            /**
             * XML parser.
             */
            XMLParser = /** @class */ (function () {
                function XMLParser() {
                }
                /**
                ...
                XMLParser.setAttributes = function (element, namespaceURI, attributesString) {
                    ...
                    if (attributes) {
                        ...
                        while ((match = regExp.exec(attributes))) {
                            if (match[1]) {
                                element.setAttributeNS(null, this._getAttributeName(namespaceURI, match[1]), he_1.decode(match[2] || match[3] || match[4] || ''));
                            }
                        }
