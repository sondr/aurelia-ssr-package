...
import { RenderOptions, AppInitializationOptions, AureliaPalNodeJS, AureliaPal } from '../../interface/ssr-engine/interfaces';
...
function start(options: AppInitializationOptions, requestUrl: string, headers?: any) {
  ...
  if(PLATFORM?.jsdom?.window?.location?.replace){
    PLATFORM?.jsdom?.window?.location.replace(requestUrl);
  }else if(PLATFORM?.jsdom?.location?.replace){
    PLATFORM?.jsdom?.location.replace(requestUrl);
  }
